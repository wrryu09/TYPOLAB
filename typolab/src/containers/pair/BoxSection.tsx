import BoxSet from "./BoxSet";
import { FontNameVarSet } from "@/types/types";
import TotheBox from "./TotheBox";

type BoxSectionProps = {
  displayFirstSize: number;
  koreanFont: FontNameVarSet;
  displayScndSize: number;
  latinFont: FontNameVarSet;
};

const BoxSection = ({
  displayFirstSize,
  koreanFont,
  displayScndSize,
  latinFont,
}: BoxSectionProps) => {
  return (
    <>
      <div className="mb-8">
        <h1 className={`subTitleStyle mb-2`}>ADD TO YOUR BOX</h1>
        <div className="break-keep text-sm">
          <p>+ 버튼을 눌러 박스에 선택한 글꼴 정보를 저장해 보세요!</p>
          <p>BOX 페이지에서 확인할 수 있습니다.</p>
        </div>
      </div>
      <div className="mobile:mb-4 mb-8 mobile:py-8 w-full py-20 border-2 border-lightGrey rounded-3xl shadow-xl">
        <div className="flex flex-col items-center gap-y-6">
          {/* 1st set */}
          <BoxSet displaySize={displayFirstSize} font={koreanFont} boxNum={1} />

          {/* 2nd set */}
          <BoxSet displaySize={displayScndSize} font={latinFont} boxNum={2} />
        </div>
      </div>
      <TotheBox />
    </>
  );
};

export default BoxSection;
